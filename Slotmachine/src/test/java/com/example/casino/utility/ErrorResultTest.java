...
class ErrorResultTest {
    ...
    @Test
    @DisplayName("Failure with extreme/random values")
    void failure_ShouldHandleExtremeData() {
        //setup
        String extremeError = "ERROR_".repeat(1000);

        //test
        ErrorResult<String> result = ErrorResult.failure(extremeError);

        //assert
        assertTrue(result.isFailure());
        assertEquals(extremeError, result.getFailureData().get());
    }

    @Test
    @DisplayName("Failure with null data")
    void failure_ShouldHandleNullData() {
        //test
        ErrorResult<String> result = ErrorResult.failure(null);

        //assert
        assertTrue(result.isFailure());
        assertFalse(result.getFailureData().isPresent());
    }
